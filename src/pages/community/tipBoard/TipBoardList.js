// src/pages/community/tipBoard/TipBoardList.js

import React, { useContext, useEffect, useState } from 'react';
import { useNavigate, useSearchParams, useLocation } from 'react-router-dom';
import apiClient from '../../../utils/axios'; // axios 인스턴스
import { AuthContext } from '../../../AuthProvider';
import styles from './TipBoardList.module.css';
...
function TipBoardList() {
  ...
  const handleSearch = () => {
    ...
    navigate(`/community/tipBoard?${query.toString()}`);
  ...
  const handleWriteClick = () => {
    ...
    navigate('/community/tipBoard/write');
  };

  const handleClick = (id) => {
    navigate(`/community/tipBoard/${id}`);
  };
  ...
  // 팁게시판 경로 클릭 시 검색 초기화 (현재 페이지에서 다시 눌러도 리셋)
  useEffect(() => {
    if (location.pathname === '/community/tipBoard') {
  ...
  useEffect(() => {
    ...
    console.log('💡 요청 URL:', `/board/tip/list?${params.toString()}`);

    apiClient
      .get(`/board/tip/list?${params.toString()}`)
      .then((res) => {
        setPostList(res.data.list);
        setPagingInfo(res.data.paging);
      })
      .catch((err) => console.error('팁게시판 목록 조회 실패:', err));
  ...
  return (
    <div className={styles.container}>
      <div className={styles.titleWrapper}>
        <h2 className={styles.sectionTitle}>📢 팁게시판</h2>
        <p className={styles.sectionSubtitle}>반려동물 관리 꿀팁부터 생활 노하우까지 다양하게 나눠보세요 🐾</p>
      </div>
      {/* 상단 제목 + 검색/정렬/글쓰기 */}
