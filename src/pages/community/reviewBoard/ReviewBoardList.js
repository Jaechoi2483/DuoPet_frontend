...
import styles from './ReviewBoardList.module.css';
...
function ReviewBoardList() {
  ...
  const handleSearch = () => {
    ...
    navigate(`/community/reviewBoard?${query.toString()}`);
  ...
  const handleWriteClick = () => {
    ...
    navigate('/community/reviewBoard/write');
  };

  const handleClick = (id) => {
    navigate(`/community/reviewBoard/${id}`);
  };
  ...
  // 후기게시판 경로 클릭 시 검색 초기화 (현재 페이지에서 다시 눌러도 리셋)
  useEffect(() => {
    if (location.pathname === '/community/reviewBoard') {
  ...
  useEffect(() => {
    ...
    console.log('💡 요청 URL:', `/board/review/list?${params.toString()}`);

    apiClient
      .get(`/board/review/list?${params.toString()}`)
      .then((res) => {
        setPostList(res.data.list);
        setPagingInfo(res.data.paging);
      })
      .catch((err) => console.error('후기게시판 목록 조회 실패:', err));
  ...
  return (
    <div className={styles.container}>
      <div className={styles.titleWrapper}>
        <h2 className={styles.sectionTitle}>📢 후기게시판</h2>
        <p className={styles.sectionSubtitle}>입양 후기, 장난감, 사료 등 다양한 이야기를 나눠보세요 🐶🐱</p>
      </div>
      {/* 상단 제목 + 검색/정렬/글쓰기 */}
