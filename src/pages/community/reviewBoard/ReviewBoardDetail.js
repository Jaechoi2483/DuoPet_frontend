// src/pages/community/reviewBoard/ReviewBoardDetail.js
...
import styles from './ReviewBoardDetail.module.css';

// 게시글 상세 보기 페이지 (후기 게시판)
function ReviewBoardDetail() {
  ...
  // 게시글 수정/목록 이동
  const handleEdit = () => navigate(`/community/reviewBoard/edit/${id}`);
  const handleBackToList = () => navigate('/community/Board');

  // 게시글 삭제
  const handleDelete = async () => {
    if (!window.confirm('정말로 이 게시글을 삭제하시겠습니까?')) return;
    try {
      const response = await secureApiRequest(`/board/review/${id}`, { method: 'DELETE' });
  ...
  const handleLike = async () => {
    ...
    try {
      const res = await apiClient.post(`/board/review/like/${id}`, null, {
        headers: {
  ...
  const handleBookmark = async () => {
    ...
    try {
      const res = await apiClient.post(`/board/review/bookmark/${id}`, null, {
        headers: {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        // 2. 게시글 상세 정보 가져오기
        const res = await apiClient.get(`/board/review/detail/${id}`);
        setPost(res.data);
        ...
        if (validAccessToken && validRefreshToken) {
          try {
            const likeRes = await apiClient.get(`/board/review/like/${id}/status`, {
              headers: {
                Authorization: `Bearer ${validAccessToken}`,
                RefreshToken: `Bearer ${validRefreshToken}`,
              },
          ...
          try {
            const bookmarkRes = await apiClient.get(`/board/review/bookmark/${id}/status`, {
              headers: {
                Authorization: `Bearer ${validAccessToken}`,
                RefreshToken: `Bearer ${validRefreshToken}`,
              },
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.tagHeader}>
        <span className={styles.badge}>후기게시판</span>
